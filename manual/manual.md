...
| layer     | rect をレイヤーに所属させる場合、その名前をキーワードで指定します。<br> \
詳細は「[](#レイヤー)」を参照してください。           |
...
| layer     | circle をレイヤーに所属させる場合、その名前をキーワードで指定します。<br> \
詳細は「[](#レイヤー)」を参照してください。           |
...
| layer     | ellipse をレイヤーに所属させる場合、その名前をキーワードで指定します。<br> \
詳細は「[](#レイヤー)」を参照してください。           |
...
| layer     | polygon をレイヤーに所属させる場合、その名前をキーワードで指定します。<br> \
詳細は「[](#レイヤー)」を参照してください。           |
...
| end1,end2 | line の始端・終端に矢印などの装飾を付ける場合は指定します。<br> \
詳細は「[](#終端マーク)」を参照してください。 |
| layer     | line をレイヤーに所属させる場合、その名前をキーワードで指定します。<br> \
詳細は「[](#レイヤー)」を参照してください。           |
...
| layer           | arc をレイヤーに所属させる場合、その名前をキーワードで指定します。<br> \
詳細は「[](#レイヤー)」を参照してください。           |
...
| layer     | path をレイヤーに所属させる場合、その名前をキーワードで指定します。<br> \
詳細は「[](#レイヤー)」を参照してください。           |
...
| パラメータ | 説明                                                                           |
|:==========|:--------------------------------------------------------------------------------------|
| from      | コネクタの接続元となる要素の ID をキーワードで指定します。 |
| to        | コネクタの接続先となる要素の ID をキーワードで指定します。 |
| style     | 接続線の引き方を指定します。省略した場合のデフォルト値は :CC です。<br> \
これは「中心から中心を直線で結ぶ」引き方になります。それ以外の<br> \
指定方法については後述します。 |
| spacing   | 2 回以上折れ曲がる接続線について、「自由な線分の位置」を調整するための<br> \
パラメータです。詳細は後述します。 |
| label     | コネクタにラベルをつける場合に指定します。<br> \
詳細は「[](#ラベル)」を参照してください。 |
| stroke    | コネクタを描画する線を指定します。<br> \
詳細は「[](#ストローク)」を参照してください。         |
| end1,end2 | コネクタの始端・終端に矢印などの装飾を付ける場合は指定します。<br> \
詳細は「[](#終端マーク)」を参照してください。 |
| layer     | コネクタをレイヤーに所属させる場合、その名前をキーワードで指定します。<br> \
詳細は「[](#レイヤー)」を参照してください。           |
| filter    | コネクタの描画にフィルタ効果を適用したい場合、その名前を指定します。<br> \
詳細は「[](#フィルタ)」を参照してください。 |
| id        | コネクタに ID を付与したい場合、その名前をキーワードで指定します。<br> \
詳細は「[](#IDと参照)」を参照してください。            |
...
　style パラメータを使って接続点を指定すると、コネクタは接続対象の図形要素の位置関係を調べて適切な
折線を描画します（:CC 以外の場合）。たとえば、以下は `:style :LR` でコネクタによる接続を行なって
います。
...
　この、位置関係を考慮した接続線の自動決定は、接続対象でない他の図形要素の位置は「考慮しない」ことに
...
いる線分は動かせませんが、残り 3 つの線分はそれぞれ x 軸方向または y 軸方向に移動させられそうなことが
わかるでしょう。つまり、以下で赤くした部分です。
