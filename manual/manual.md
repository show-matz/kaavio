...
参照するものです。これには９種類あり、その名前と具体的な場所は以下の通りです
...
　connect からみれば rect は 2 つ前なので $2、circle は 1 つ前なので $1 で
参照し、$1.id とすることでその ID を参照しています。結局、これは `:x` や `:y` を
指定しているのと同じことになります。

　上記の説明からすると奇妙に思われるかもしれませんが、上記の状態になっていれば、
rect / circle から ID 指定は除去しても問題ありません。つまり、以下のように
書けるということです。
...
| `id`           | 図形要素の ID を参照します。       |
| `width`        | 図形要素の幅を参照します。         |
| `height`       | 図形要素の高さを参照します。       |
| `topleft`      | 図形要素の左上の座標を参照します。 |
| `top`          | 図形要素の上端の座標を参照します。 |
| `topright`     | 図形要素の右上の座標を参照します。 |
| `left`         | 図形要素の右端の座標を参照します。 |
| `center`       | 図形要素の中心の座標を参照します。 |
| `right`        | 図形要素の左端の座標を参照します。 |
| `bottomleft`   | 図形要素の左下の座標を参照します。 |
| `bottom`       | 図形要素の下端の座標を参照します。 |
| `bottomright`  | 図形要素の右下の座標を参照します。 |
