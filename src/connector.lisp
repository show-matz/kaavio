...
(defun resolve-connector-points-for-CC (pos1 pos2 e1 e2)
  ...
  (labels ((shape-cc-point (shp x y)
			 (let ((pt (get-cc-point shp x y)))
	...
	(with-property
		(let ((x1 e1[center])
		  ...
		  (multiple-value-setq (x2 y2) (shape-cc-point e2 x1 y1))
		  (list x1 y1 x2 y2)))))

(defun resolve-connector-points-for-BB (pos1 pos2 e1 e2)
  (with-property
	  (let ((dist *default-connector-spacing*)
			(x1 (+ e1[center] (* (/ e1[width] 4) pos1)))
			(y1 e1[bottom])
			(x2 (+ e2[center] (* (/ e2[width] 4) pos2)))
			(y2 e2[bottom]))
		(labels ((points4 ()
...
(defun resolve-connector-points-for-LL (pos1 pos2 e1 e2)
  (with-property
	  (let ((dist *default-connector-spacing*)
			(x1 e1[left])
			(y1 (+ e1[middle] (* (/ e1[height] 4) pos1)))
			(x2 e2[left])
			(y2 (+ e2[middle] (* (/ e2[height] 4) pos2))))
		(labels ((points4 ()
			  ...
			  (if (< e2[right] e1[left])
				  ...
				  (if (< e1[right] e2[left])
					  ...
					  (let ((lx (- (min e1[left]  e2[left])  dist))
						...
						;; -> 2x1 + 2x2 < 2rx + 2lx
						;; -> x1 + x2 < rx + lx
						(points6 (if (< (+ x1 x2) (+ rx lx)) lx rx) (+ y1 dist) (- y2 dist))))))))))
			  ...
			  (if (< e1[bottom] e2[top])
				  ...
				  (if (< e2[bottom] e1[top])
					  ...
					  (let ((ty (- (min e1[top]  e2[top])  dist))
						...
						;; -> 2y1 + 2y2 < 2ty + 2by
						;; -> y1 + y2 < ty + by
						(points6 (if (< (+ y1 y2) (+ ty by)) ty by) (- x1 dist) (+ x2 dist))))))))))
